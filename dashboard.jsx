// Estrela Potiguar B2B Dashboard MVP - Static Fallback
// Compiled dynamically by Babel in the browser
...
function DashboardApp() {
  ...
  // Design System global instances
  const { Button, IconButton, Input, Badge, Tag, Card, Switch, Tabs, Rating, Avatar } = window.EstrelaPotiguarDesignSystem_69a35c;
      ...
      <div style={{
        ...
        <div style={{
          ...
          {activeTab === 'consultor-ia' && (
            <div style={{ height: '550px', display: 'flex', flexDirection: 'column', position: 'relative' }}>
              {!isPremium && (
                <div style={{ position: 'absolute', inset: 0, zIndex: 20, backdropFilter: 'blur(8px)', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '24px' }}>
              ...
              <div style={{ flex: 1, display: 'flex', background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', opacity: isPremium ? 1 : 0.25, pointerEvents: isPremium ? 'auto' : 'none' }}>
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
                  ...
                  <div style={{ padding: '18px 24px', borderTop: '1px solid var(--border-subtle)', display: 'flex', gap: '12px', alignItems: 'center' }}>
                    <Input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Pergunte ao consultor de IA..." style={{ flex: 1 }} onKeyDown={e => { if (e.key === 'Enter') sendChatMessage(chatInput); }} />
                    <Button variant="primary" onClick={() => sendChatMessage(chatInput)}>Enviar</Button>
                  </div>
                </div>
              </div>
            </div>
              ...
              <Card padding="lg" style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  {TOURIST_DEMOGRAPHICS.budget.map((item, idx) => (
                    <div key={idx} style={{ background: item.featured ? 'var(--accent-soft)' : 'var(--surface-inset)', border: `1px solid ${item.featured ? 'var(--accent)' : 'var(--border-subtle)'}`, borderRadius: 'var(--radius-lg)', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <Card padding="lg" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    <div style={{ height: '120px', position: 'relative', marginTop: '10px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                      <svg viewBox="0 0 100 35" style={{ width: '100%', height: '80%', overflow: 'visible' }}>
                        <defs>
                          <linearGradient id="glow-loss-static" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor="var(--accent)" stopOpacity="0.4" />
                            <stop offset="100%" stopColor="var(--accent)" stopOpacity="0.0" />
                          </linearGradient>
                        </defs>
                        {/* Area */}
                        <path d="M 0 35 Q 15 32 30 20 T 60 5 T 80 25 T 100 10 L 100 35 Z" fill="url(#glow-loss-static)" />
                        {/* Line */}
                  ...
                  <Card key={review.id} padding="md" style={{ display: 'flex', flexDirection: 'column', gap: '12px', borderLeft: `4px solid ${review.type === 'estrela' ? 'var(--accent)' : 'var(--border-strong)'}` }}>
                    ...
                    <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: '12px', marginTop: '4px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      {aiResponses[review.id] ? (
                        ...
                        <Button variant="secondary" size="sm" disabled={respondingReviewId === review.id} onClick={() => generateAiReviewResponse(review.id, review.author, review.text, review.type)} style={{ alignSelf: 'flex-start' }}>
